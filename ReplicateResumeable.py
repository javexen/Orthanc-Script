...
sources_studies = DoGetJson('%s/instances' % SOURCE)
targer_instances = DoGetJson('%s/instances' % TARGET)

instance_to_move = list(set(sources_studies) - set(targer_instances))
number_to_move= len(instance_to_move)
print(number_to_move)
i=0
for instance in instance_to_move:
    i+=1
    print('sending image '+ str(i) +' on '+ str(number_to_move))
    dicom = DoGetString('%s/instances/%s/file' % (SOURCE, instance))
